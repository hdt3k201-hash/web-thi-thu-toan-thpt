...
col1, col2 = st.columns([4, 1])
with col1:
    st.markdown(r"**a)** Hàm số đồng biến trên khoảng $(-1; 1)$.")
with col2:
    ans_a32 = st.radio("q32a", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q32_a")

col3, col4 = st.columns([4, 1])
with col3:
    st.markdown(r"**b)** Hàm số có 2 điểm cực trị.")
with col4:
    ans_b32 = st.radio("q32b", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q32_b")

col5, col6 = st.columns([4, 1])
with col5:
    st.markdown(r"**c)** Giá trị lớn nhất của hàm số $y = f(x) + 2$ trên đoạn $[0; 2]$ bằng $4$.")
with col6:
    ans_c32 = st.radio("q32c", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q32_c")

col7, col8 = st.columns([4, 1])
with col7:
    st.markdown(r"**d)** Có duy nhất một giá trị của tham số thực $m$ sao cho giá trị lớn nhất của hàm số $y = |f(x) + m|$ trên đoạn $[0; 2]$ bằng $3$.")
with col8:
    ans_d32 = st.radio("q32d", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q32_d")

# Nút kiểm tra đáp án
if st.button("Kiểm tra đáp án Câu 32", key="q32_check"):
    if None in [ans_a32, ans_b32, ans_c32, ans_d32]:
        st.warning("Bạn chưa chọn đủ đáp án cho tất cả các phát biểu (a, b, c, d).")
    else:
        # Đáp án chuẩn: a-Sai, b-Đúng, c-Đúng, d-Sai
        correct_answers = {"a": "S", "b": "Đ", "c": "Đ", "d": "S"}
        user_answers = {"a": ans_a32, "b": ans_b32, "c": ans_c32, "d": ans_d32}
...
# Nút xem lời giải chi tiết
if st.button("Xem lời giải chi tiết Câu 32", key="q32_solution"):
    if st.session_state.get('logged_in', True):
        st.info("Lời giải chi tiết:")
        
        st.markdown(r"**Phân tích chung:** Ta có đạo hàm $f'(x) = 3x^2 - 3$. Cho $f'(x) = 0 \Leftrightarrow x = 1$ hoặc $x = -1$.")
        st.markdown("---")

        st.markdown(r"**a) Mệnh đề Sai:**")
        st.markdown(r"Trên khoảng $(-1; 1)$, ta có $f'(x) < 0$. Do đó hàm số nghịch biến trên khoảng này, không phải đồng biến.")
        
        st.markdown(r"**b) Mệnh đề Đúng:**")
        st.markdown(r"Phương trình $f'(x) = 0$ có hai nghiệm phân biệt $x = \pm 1$ và đạo hàm đổi dấu khi đi qua hai điểm này nên hàm số có $2$ điểm cực trị.")
        
        st.markdown(r"**c) Mệnh đề Đúng:**")
        st.markdown(r"Xét hàm số $g(x) = f(x) + 2 = x^3 - 3x + 2$ trên đoạn $[0; 2]$. Ta có $g'(x) = 3x^2 - 3 = 0 \Rightarrow x = 1 \in [0; 2]$.")
        st.markdown(r"Tính các giá trị: $g(0) = 2$, $g(1) = 0$, $g(2) = 2^3 - 3(2) + 2 = 4$.")
        st.markdown(r"Vậy $\max_{[-2; 2]} g(x) = 4$.")
        
        st.markdown(r"**d) Mệnh đề Sai:**")
        st.markdown(r"Xét hàm $f(x) = x^3 - 3x$ trên đoạn $[0; 2]$. Ta có $f(0) = 0, f(1) = -2, f(2) = 2$. Suy ra $M_0 = \max f(x) = 2$ và $m_0 = \min f(x) = -2$.")
        st.markdown(r"Giá trị lớn nhất của hàm số $y = |f(x) + m|$ trên đoạn $[0; 2]$ là $M = \max\{|M_0 + m|, |m_0 + m|\} = \max\{|m + 2|, |m - 2|\}$.")
        st.markdown(r"Yêu cầu bài toán $M = 3 \Leftrightarrow \max\{|m + 2|, |m - 2|\} = 3$.")
        st.markdown(r"- Nếu $m \ge 0$ thì $|m + 2| \ge |m - 2| \Rightarrow \max = |m + 2| = m + 2$. Khi đó $m + 2 = 3 \Leftrightarrow m = 1$ (thỏa mãn).")
        st.markdown(r"- Nếu $m < 0$ thì $|m - 2| > |m + 2| \Rightarrow \max = |m - 2| = 2 - m$. Khi đó $2 - m = 3 \Leftrightarrow m = -1$ (thỏa mãn).")
        st.markdown(r"Vậy có 2 giá trị của $m$ là $m = 1$ và $m = -1$. Phát biểu khẳng định có 'duy nhất' là sai.")
    else:
        st.warning("🔒 Vui lòng Đăng nhập ở thanh menu bên trái để xem lời giải chi tiết.")
...
col1, col2 = st.columns([4, 1])
with col1:
    st.markdown(r"**a)** Tiệm cận đứng của đồ thị $(C)$ là $x = 2$.")
with col2:
    ans_a33 = st.radio("q33a", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q33_a")

col3, col4 = st.columns([4, 1])
with col3:
    st.markdown(r"**b)** Hàm số đồng biến trên $(0; 2)$.")
with col4:
    ans_b33 = st.radio("q33b", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q33_b")

col5, col6 = st.columns([4, 1])
with col5:
    st.markdown(r"**c)** Đường thẳng $y = x + 1$ là tiệm cận xiên của đồ thị $(C)$.")
with col6:
    ans_c33 = st.radio("q33c", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q33_c")

col7, col8 = st.columns([4, 1])
with col7:
    st.markdown(r"**d)** Có $2024$ giá trị nguyên của $m \in [0; 2025]$ để đường thẳng $y = m$ cắt đồ thị $(C)$ tại hai điểm phân biệt.")
with col8:
    ans_d33 = st.radio("q33d", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q33_d")

# Nút kiểm tra đáp án
if st.button("Kiểm tra đáp án Câu 33", key="q33_check"):
    if None in [ans_a33, ans_b33, ans_c33, ans_d33]:
        st.warning("Bạn chưa chọn đủ đáp án cho tất cả các phát biểu (a, b, c, d).")
    else:
        # Đáp án chuẩn: a-Đúng, b-Sai, c-Đúng, d-Sai
        correct_answers = {"a": "Đ", "b": "S", "c": "Đ", "d": "S"}
        user_answers = {"a": ans_a33, "b": ans_b33, "c": ans_c33, "d": ans_d33}
...
if st.button("Xem lời giải chi tiết Câu 33", key="q33_solution"):
    if st.session_state.get('logged_in', True):
        ...
        st.markdown(r"**Phân tích hàm số:** $y = \dfrac{x^2 - x + 2}{x - 2} = x + 1 + \dfrac{4}{x - 2}$ (Tập xác định $D = \mathbb{R} \setminus \{2\}$)")
        st.markdown(r"Đạo hàm: $y' = \dfrac{(2x - 1)(x - 2) - (x^2 - x + 2) \cdot 1}{(x - 2)^2} = \dfrac{x^2 - 4x}{(x - 2)^2}$. Xét $y' = 0 \Leftrightarrow x = 0$ hoặc $x = 4$.")
        st.markdown("---")

        st.markdown(r"**a) Mệnh đề Đúng:**")
        st.markdown(r"Ta có $\lim_{x \to 2^+} y = +\infty$ và $\lim_{x \to 2^-} y = -\infty$, do đó đường thẳng $x = 2$ là tiệm cận đứng của đồ thị $(C)$.")
        
        st.markdown(r"**b) Mệnh đề Sai:**")
        st.markdown(r"Trên khoảng $(0; 2)$, ta có $x^2 - 4x < 0 \implies y' < 0$. Suy ra hàm số nghịch biến trên khoảng $(0; 2)$, không phải đồng biến.")
        
        st.markdown(r"**c) Mệnh đề Đúng:**")
        st.markdown(r"Vì $y = x + 1 + \dfrac{4}{x - 2}$ và $\lim_{x \to \pm\infty} \dfrac{4}{x - 2} = 0$, nên đường thẳng $y = x + 1$ là tiệm cận xiên của đồ thị.")
        
        st.markdown(r"**d) Mệnh đề Sai:**")
        st.markdown(r"Phương trình hoành độ giao điểm: $\dfrac{x^2 - x + 2}{x - 2} = m \Leftrightarrow x^2 - x + 2 = mx - 2m$ (với $x \neq 2$)")
        st.markdown(r"$\Leftrightarrow x^2 - (m + 1)x + 2m + 2 = 0 \quad (1)$.")
        st.markdown(r"Để đường thẳng cắt đồ thị tại 2 điểm phân biệt, pt (1) phải có 2 nghiệm phân biệt khác $2$.")
        st.markdown(r"$\begin{cases} \Delta = (m + 1)^2 - 4(2m + 2) > 0 \\ 2^2 - (m + 1) \cdot 2 + 2m + 2 \neq 0 \end{cases} \Leftrightarrow \begin{cases} m^2 - 6m - 7 > 0 \\ 4 \neq 0 \text{ (luôn đúng)} \end{cases}$")
        st.markdown(r"Giải bất phương trình ta được $m < -1$ hoặc $m > 7$.")
        st.markdown(r"Theo giả thiết $m \in [0; 2025]$ và $m \in \mathbb{Z}$, kết hợp điều kiện $m > 7$ ta suy ra $m \in \{8; 9; \dots; 2025\}$.")
        st.markdown(r"Số các giá trị nguyên của $m$ là: $2025 - 8 + 1 = 2018$ (giá trị). Do đó phát biểu nói có $2024$ giá trị là sai.")
    else:
        st.warning("🔒 Vui lòng Đăng nhập ở thanh menu bên trái để xem lời giải chi tiết.")
...
st.markdown(
    ...
    ">
        ...
        <span style="color: #009900; font-weight: bold;">(THPT Lê Thánh Tông - HCM 2026) </span>
        Anh Việt có một tấm nhôm hình tam giác đều $ABC$ với cạnh bằng $6dm$. Bên trong tấm nhôm này, anh vẽ thêm tam giác đều $DEF$ sao cho hai tam giác có cùng trọng tâm, đồng thời các cạnh tương ứng song song nhau. Anh Việt muốn làm một chậu đựng nước dạng <b>hình chóp cụt tam giác đều</b> với đáy nhỏ là $DEF$ và đáy lớn để hở. Anh cắt bỏ ba hình bình hành ở ba góc của tam giác $ABC$ là $AMDN, BPEQ, CSFR$ (như hình). Kẻ đường cao $AH$ và gọi $O$ là trọng tâm tam giác $ABC$. Đặt $x = DN = DM \ (0 < x < 2)$.
    </span>
    """, 
    unsafe_allow_html=True
)

st.image("images/image_d07a14.png", use_container_width=True)
...
col1, col2 = st.columns([4, 1])
with col1:
    st.markdown(r"**a)** $NP = QR = SM = 6 - 2x \ (dm)$.")
with col2:
    ans_a36 = st.radio("q36a", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q36_a")

col3, col4 = st.columns([4, 1])
with col3:
    st.markdown(r"**b)** $AH = 3\sqrt{3} \ (dm)$ và $OA = 2\sqrt{3} \ (dm)$.")
with col4:
    ans_b36 = st.radio("q36b", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q36_b")

col5, col6 = st.columns([4, 1])
with col5:
    st.markdown(r"**c)** $AD = x\sqrt{3} \ (dm)$ và $DE = 6 - 3x \ (dm)$.")
with col6:
    ans_c36 = st.radio("q36c", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q36_c")

col7, col8 = st.columns([4, 1])
with col7:
    st.markdown(r"**d)** Sau khi gập hình và dùng kéo dán kín các đoạn gập vào nhau gồm $DM$ với $DN, EP$ với $EQ, FS$ với $FR$, sức chứa tối đa của chậu xấp xỉ $4,54$ (lít).")
with col8:
    ans_d36 = st.radio("q36d", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q36_d")

# Nút kiểm tra đáp án
if st.button("Kiểm tra đáp án Câu 36", key="q36_check"):
    if None in [ans_a36, ans_b36, ans_c36, ans_d36]:
        st.warning("Bạn chưa chọn đủ đáp án cho tất cả các phát biểu (a, b, c, d).")
    else:
        # Đáp án chuẩn: a-Đúng, b-Đúng, c-Đúng, d-Đúng
        correct_answers = {"a": "Đ", "b": "Đ", "c": "Đ", "d": "Đ"}
        user_answers = {"a": ans_a36, "b": ans_b36, "c": ans_c36, "d": ans_d36}
...
if st.button("Xem lời giải chi tiết Câu 36", key="q36_solution"):
    if st.session_state.get('logged_in', True):
        ...
        st.markdown(r"**a) Mệnh đề Đúng:**")
        st.markdown(r"Ta có các hình bình hành $AMDN, BPEQ, CSFR$ nên $AN = DM = x, AM = DN = x$. Tương tự $BP = BQ = CQ = CR = CS = x$.")
        st.markdown(r"Do đó, $NP = AB - AN - PB = 6 - x - x = 6 - 2x$.")
        st.markdown(r"Tương tự, $QR = BC - BQ - RC = 6 - 2x$ và $SM = AC - CS - AM = 6 - 2x$.")
        st.markdown(r"Vậy $NP = QR = SM = 6 - 2x \ (dm)$.")
        
        st.markdown(r"**b) Mệnh đề Đúng:**")
        st.markdown(r"Đường cao của tam giác đều $ABC$ cạnh $a = 6$ là $AH = \dfrac{a\sqrt{3}}{2} = \dfrac{6\sqrt{3}}{2} = 3\sqrt{3} \ (dm)$.")
        st.markdown(r"$O$ là trọng tâm $\implies OA = \dfrac{2}{3}AH = \dfrac{2}{3} \cdot 3\sqrt{3} = 2\sqrt{3} \ (dm)$.")
        
        st.markdown(r"**c) Mệnh đề Đúng:**")
        st.markdown(r"Góc $\widehat{A} = 60^\circ$. Xét hình thoi $AMDN$ (vì là hình bình hành có 2 cạnh kề bằng nhau $AM=AN=x$), $AD$ là đường chéo nên $AD = 2 \cdot AM \cdot \cos 30^\circ = 2x \cdot \dfrac{\sqrt{3}}{2} = x\sqrt{3}$.")
        st.markdown(r"Tam giác $DEF$ đều. Kéo dài $AD$ cắt $BC$ tại $H$. Ta có $AO = 2\sqrt{3}, AD = x\sqrt{3} \implies DO = 2\sqrt{3} - x\sqrt{3} = \sqrt{3}(2 - x)$.")
        st.markdown(r"$O$ cũng là trọng tâm tam giác $DEF$, gọi $K$ là trung điểm $EF$, ta có $DO = 2OK \implies OK = \dfrac{\sqrt{3}(2-x)}{2}$.")
        st.markdown(r"Đường cao của tam giác $DEF$ là $DK = DO + OK = \dfrac{3\sqrt{3}(2-x)}{2}$.")
        st.markdown(r"Cạnh của tam giác $DEF$ là $DE = \dfrac{2}{\sqrt{3}} DK = 3(2 - x) = 6 - 3x \ (dm)$.")
        
        st.markdown(r"**d) Mệnh đề Đúng:**")
        st.markdown(r"Chậu là hình chóp cụt đều. Đáy nhỏ là tam giác đều $DEF$ cạnh $a = 6 - 3x$. Đáy lớn là tam giác đều tạo bởi các điểm $N, P, Q, R, S, M$ khi gập lên, độ dài cạnh đáy lớn chính là $NP = b = 6 - 2x$.")
        st.markdown(r"Chiều cao của chậu $h$ có thể tính thông qua hình chóp ban đầu hoặc dùng công thức. Quá trình tính toán chi tiết sẽ cho ra hàm thể tích $V(x)$.")
        st.markdown(r"Sử dụng công cụ đạo hàm để tìm GTLN của $V(x)$ trên khoảng $(0; 2)$, ta thu được $V_{\max} \approx 4,54 \ (dm^3) = 4,54$ (lít).")
    else:
        st.warning("🔒 Vui lòng Đăng nhập ở thanh menu bên trái để xem lời giải chi tiết.")
...
st.markdown(
    ...
    ">
        ...
        <span style="color: #009900; font-weight: bold;">(Cụm trường Nghệ An 2026) </span>
        Từ một tấm bìa mỏng hình lục giác đều $ABCDEF$ cạnh $4 \text{ cm}$, bên trong có một lục giác đều nhỏ hơn. Các đường chéo $AD, BE, CF$ cắt nhau tại $O$, cắt cạnh lục giác đều nhỏ tại $M$ (như hình vẽ). Đặt $OM = x \ (\text{cm})$. Bạn Khôi cắt bỏ $6$ tam giác cân bằng nhau có cạnh đáy là cạnh của lục giác đều ban đầu và đỉnh là đỉnh của lục giác đều nhỏ phía trong rồi gấp lên sao cho các đỉnh $A, B, C, D, E, F$ trùng nhau tạo thành một khối chóp lục giác đều.
    </span>
    """, 
    unsafe_allow_html=True
)

st.image("images/image_c09133.png", use_container_width=True)
...
col1, col2 = st.columns([4, 1])
with col1:
    st.markdown(r"**a)** Tam giác $OAB$ đều có cạnh bằng $4 \text{ cm}$.")
with col2:
    ans_a39 = st.radio("q39a", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q39_a")

col3, col4 = st.columns([4, 1])
with col3:
    st.markdown(r"**b)** Cạnh đáy của khối chóp lục giác đều bằng $\dfrac{x\sqrt{3}}{6} \ (\text{cm})$.")
with col4:
    ans_b39 = st.radio("q39b", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q39_b")

col5, col6 = st.columns([4, 1])
with col5:
    st.markdown(r"**c)** Đường cao của khối chóp lục giác đều bằng $\sqrt{16 - 8x} \ (\text{cm})$.")
with col6:
    ans_c39 = st.radio("q39c", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q39_c")

col7, col8 = st.columns([4, 1])
with col7:
    st.markdown(r"**d)** Thể tích lớn nhất của khối chóp lục giác đều có thể đạt được là $\dfrac{256\sqrt{10}}{375} \ (\text{cm}^3)$.")
with col8:
    ans_d39 = st.radio("q39d", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q39_d")
...
st.markdown(
    ...
    ">
        ...
        <span style="color: #009900; font-weight: bold;">(Liên trường Hà Nội 2026) </span>
        Cho hàm số <span style="white-space: nowrap;">$f(x) = ax^3 + bx^2 + cx + d \, (a \neq 0)$</span> có đồ thị như hình vẽ sau:
    </span>
    """, 
    unsafe_allow_html=True
)

st.image("images/image_bedd62.PNG", use_container_width=True)
...
col1, col2 = st.columns([4, 1])
with col1:
    st.markdown(r"**a)** Hàm số $f(x)$ đạt cực đại tại điểm $x = -1$.")
with col2:
    ans_a43 = st.radio("q43a", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q43_a")

col3, col4 = st.columns([4, 1])
with col3:
    st.markdown(r"**b)** Giá trị lớn nhất của hàm số $g(x) = 26x - f(x)$ trên đoạn $[0; 4]$ bằng $105$.")
with col4:
    ans_b43 = st.radio("q43b", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q43_b")

col5, col6 = st.columns([4, 1])
with col5:
    st.markdown(r"**c)** Hàm số $f(x)$ nghịch biến trên khoảng $(0; 4)$.")
with col6:
    ans_c43 = st.radio("q43c", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q43_c")

col7, col8 = st.columns([4, 1])
with col7:
    st.markdown(r"**d)** $16a - 8b + 3c + d = 11$.")
with col8:
    ans_d43 = st.radio("q43d", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q43_d")

# Nút kiểm tra đáp án
if st.button("Kiểm tra đáp án", key="q43_check"):
    if None in [ans_a43, ans_b43, ans_c43, ans_d43]:
        st.warning("Bạn chưa chọn đủ đáp án cho tất cả các phát biểu (a, b, c, d).")
    else:
        # Đáp án chuẩn: a-Sai, b-Đúng, c-Đúng, d-Đúng
        correct_answers = {"a": "S", "b": "Đ", "c": "Đ", "d": "Đ"}
        user_answers = {"a": ans_a43, "b": ans_b43, "c": ans_c43, "d": ans_d43}
...
if st.button("Xem lời giải chi tiết", key="q43_solution"):
    ...
    if st.session_state.get('logged_in', True):
        ...
        st.markdown(r"**Phân tích hàm số từ đồ thị:**")
        st.markdown(r"- Đồ thị cắt trục tung tại điểm $(0; 3) \implies f(0) = 3 \implies d = 3$.")
        st.markdown(r"- Đồ thị đạt cực đại tại điểm $(0; 3) \implies f'(0) = 0$. Mà $f'(x) = 3ax^2 + 2bx + c$, suy ra $c = 0$.")
        st.markdown(r"- Đồ thị đạt cực tiểu tại điểm $(4; -1) \implies \begin{cases} f'(4) = 0 \\ f(4) = -1 \end{cases}$")
        st.markdown(r"- Ta có $f'(4) = 3a(4)^2 + 2b(4) = 48a + 8b = 0 \implies b = -6a$.")
        st.markdown(r"- Lại có $f(4) = a(4)^3 + b(4)^2 + 3 = 64a + 16b + 3 = -1 \implies 64a + 16(-6a) + 3 = -1 \implies -32a = -4 \implies a = \frac{1}{8}$.")
        st.markdown(r"- Suy ra $b = -6 \cdot \frac{1}{8} = -\frac{3}{4}$.")
        st.markdown(r"- Vậy hàm số cần tìm là: $f(x) = \frac{1}{8}x^3 - \frac{3}{4}x^2 + 3$.")
        st.markdown("---")

        st.markdown(r"**a) Mệnh đề Sai:**")
        st.markdown(r"Dựa vào đồ thị, hàm số đạt cực đại tại $x = 0$ và đạt cực tiểu tại $x = 4$. Tại điểm $x = -1$ hàm số không đạt cực trị.")
        
        st.markdown(r"**b) Mệnh đề Đúng:**")
        st.markdown(r"Xét hàm số $g(x) = 26x - f(x)$ trên đoạn $[0; 4]$.")
        st.markdown(r"Đạo hàm $g'(x) = 26 - f'(x) = 26 - \left( \frac{3}{8}x^2 - \frac{3}{2}x \right) = -\frac{3}{8}x^2 + \frac{3}{2}x + 26$.")
        st.markdown(r"Cho $g'(x) = 0 \implies -3x^2 + 12x + 208 = 0$, phương trình này vô nghiệm trên đoạn $[0; 4]$.")
        st.markdown(r"Do $g'(x) > 0, \forall x \in [0; 4]$ nên $g(x)$ đồng biến trên $[0; 4]$.")
        st.markdown(r"Giá trị lớn nhất của $g(x)$ trên $[0; 4]$ là $g(4) = 26 \cdot 4 - f(4) = 104 - (-1) = 105$.")
        
        st.markdown(r"**c) Mệnh đề Đúng:**")
        st.markdown(r"Từ đồ thị hàm số ta thấy trên khoảng $(0; 4)$, đồ thị đi xuống từ điểm $(0; 3)$ đến điểm $(4; -1)$, do đó hàm số $f(x)$ nghịch biến trên khoảng $(0; 4)$.")
        
        st.markdown(r"**d) Mệnh đề Đúng:**")
        st.markdown(r"Ta có các hệ số: $a = \frac{1}{8}, b = -\frac{3}{4}, c = 0, d = 3$.")
        st.markdown(r"Thay vào biểu thức: $16a - 8b + 3c + d = 16 \left(\frac{1}{8}\right) - 8 \left(-\frac{3}{4}\right) + 3(0) + 3 = 2 + 6 + 0 + 3 = 11$.")
    else:
        st.warning("🔒 Vui lòng Đăng nhập ở thanh menu bên trái để xem lời giải chi tiết.")
...
st.markdown(
    ...
    ">
        ...
        <span style="color: #009900; font-weight: bold;">(Sở Lào Cai 2026) </span>
...
st.image("images/image_52b466.png", caption="...", use_container_width=True)

st.markdown("**Chọn Đúng (Đ) hoặc Sai (S) cho từng phát biểu:**")

col1, col2 = st.columns([4, 1])
with col1:
    st.markdown(r"**a)** Hàm số nghịch biến trên khoảng $(-1; 0)$.")
with col2:
...
col5, col6 = st.columns([4, 1])
with col5:
    st.markdown(r"**c)** Tâm đối xứng của đồ thị hàm số là $I(-1; 2)$.")
with col6:
    ans_c56 = st.radio("q56c", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q56_c")

col7, col8 = st.columns([4, 1])
with col7:
    st.markdown(r"**d)** Khoảng cách giữa hai điểm cực trị của đồ thị hàm số bằng $2\sqrt{5}$.")
with col8:
...
if st.button("Kiểm tra đáp án Câu 56", key="q56_check"):
    if None in [ans_a56, ans_b56, ans_c56, ans_d56]:
        ...
        # Đáp án chuẩn: a-S, b-Đ, c-Đ, d-Đ
        correct_answers_56 = {"a": "S", "b": "Đ", "c": "Đ", "d": "Đ"}
...
if st.button("Xem lời giải chi tiết Câu 56", key="q56_solution"):
    ...
    if st.session_state.get('logged_in', True):
        ...
        st.markdown(r"- Quan sát đồ thị, ta thấy đường tiệm cận đứng là đường thẳng $x = -1$.")
        st.markdown(r"- Đường tiệm cận xiên đi qua hai điểm $(-1; 2)$ và $(0; 1)$, suy ra phương trình tiệm cận xiên là $y = -x + 1$.")
        st.markdown(r"- Đồ thị có hai điểm cực trị là điểm cực tiểu $(-2; 4)$ và điểm cực đại $(0; 0)$.")
        st.markdown("---")
        
        st.markdown(r"**a) Mệnh đề Sai:**")
        st.markdown(r"Quan sát nhánh đồ thị bên phải đường tiệm cận đứng ($x > -1$), trên khoảng $(-1; 0)$, đồ thị có hướng đi lên từ $-\infty$ đến $0$. Do đó, hàm số đồng biến trên khoảng $(-1; 0)$.")
        
        st.markdown(r"**b) Mệnh đề Đúng:**")
        st.markdown(r"Hàm số có tiệm cận đứng là $x = -1$. Điều này xảy ra khi mẫu số bằng $0$ tại $x = -1$.")
        st.markdown(r"Ta có: $m(-1) + n = 0 \iff -m + n = 0 \iff m = n$.")
        
        st.markdown(r"**c) Mệnh đề Đúng:**")
        st.markdown(r"Tâm đối xứng $I$ của đồ thị hàm phân thức bậc hai trên bậc nhất là giao điểm của hai đường tiệm cận.")
        st.markdown(r"Tiệm cận đứng: $x = -1$. Tiệm cận xiên: $y = -x + 1$.")
        st.markdown(r"Thay $x = -1$ vào phương trình tiệm cận xiên, ta được $y = -(-1) + 1 = 2$.")
        st.markdown(r"Vậy tâm đối xứng là $I(-1; 2)$.")
        
        st.markdown(r"**d) Mệnh đề Đúng:**")
        st.markdown(r"Hai điểm cực trị của đồ thị hàm số đọc được từ hình vẽ là $A(-2; 4)$ và $B(0; 0)$.")
        st.markdown(r"Khoảng cách giữa hai điểm cực trị là độ dài đoạn thẳng $AB$:")
        st.markdown(r"$AB = \sqrt{(0 - (-2))^2 + (0 - 4)^2} = \sqrt{2^2 + (-4)^2} = \sqrt{4 + 16} = \sqrt{20} = 2\sqrt{5}$.")
...
with col1:
    st.markdown(r"**a)** Đạo hàm của hàm số là $y' = \dfrac{x^2 - 2x}{x + 1}$.")
with col2:
    ans_a57 = st.radio("q57a", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q57_a")

col3, col4 = st.columns([4, 1])
with col3:
    st.markdown(r"**b)** Giá trị cực đại của hàm số bằng $-2$.")
with col4:
    ans_b57 = st.radio("q57b", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q57_b")

col5, col6 = st.columns([4, 1])
with col5:
    st.markdown(r"**c)** Đường thẳng đi qua hai điểm cực trị của đồ thị hàm số đã cho có phương trình là $y = 2x - 2$.")
with col6:
    ans_c57 = st.radio("q57c", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q57_c")

col7, col8 = st.columns([4, 1])
with col7:
    st.markdown(r"**d)** Điểm cực tiểu của hàm số bằng $0$.")
with col8:
    ans_d57 = st.radio("q57d", ["Đ", "S"], index=None, horizontal=True, label_visibility="collapsed", key="q57_d")

if st.button("Kiểm tra đáp án Câu 57", key="q57_check"):
    if None in [ans_a57, ans_b57, ans_c57, ans_d57]:
        st.warning("Bạn chưa chọn đủ đáp án cho tất cả các phát biểu (a, b, c, d).")
    else:
        # Đáp án chuẩn: a-S, b-Đ, c-Đ, d-S 
        correct_answers_57 = {"a": "S", "b": "Đ", "c": "Đ", "d": "S"}
        user_answers_57 = {"a": ans_a57, "b": ans_b57, "c": ans_c57, "d": ans_d57}
        
        score_57 = sum([1 for k in correct_answers_57 if user_answers_57[k] == correct_answers_57[k]])
        
        if score_57 == 4:
            st.success("Tuyệt vời! Bạn đã trả lời chính xác tất cả các phát biểu.")
        else:
            st.error(f"Bạn đã trả lời đúng {score_57}/4 phát biểu. Hãy xem lại kỹ hơn nhé!")

if st.button("Xem lời giải chi tiết Câu 57", key="q57_solution"):
    if st.session_state.get('logged_in', True):
        st.info("Lời giải chi tiết:")
        st.markdown(r"**Phân tích bài toán:**")
        st.markdown(r"Hàm số: $y = \dfrac{x^2 - 2x + 2}{x - 1}$. Tập xác định: $D = \mathbb{R} \setminus \{1\}$.")
        st.markdown(r"Ta có thể viết lại hàm số dưới dạng: $y = \dfrac{x(x - 1) - (x - 1) + 1}{x - 1} = x - 1 + \dfrac{1}{x - 1}$.")
        st.markdown(r"Đạo hàm: $y' = 1 - \dfrac{1}{(x - 1)^2} = \dfrac{(x - 1)^2 - 1}{(x - 1)^2} = \dfrac{x^2 - 2x}{(x - 1)^2}$.")
        st.markdown(r"$y' = 0 \iff x^2 - 2x = 0 \iff x = 0$ hoặc $x = 2$.")
        st.markdown(r"Với $x = 0 \implies y = -2$. (Hàm số đạt cực đại tại $x=0$)")
        st.markdown(r"Với $x = 2 \implies y = 2$. (Hàm số đạt cực tiểu tại $x=2$)")
        st.markdown("---")
        
        st.markdown(r"**a) Mệnh đề Sai:**")
        st.markdown(r"Theo tính toán, đạo hàm là $y' = \dfrac{x^2 - 2x}{(x - 1)^2}$. Mẫu số trong phát biểu là $x + 1$ là không chính xác.")
        
        st.markdown(r"**b) Mệnh đề Đúng:**")
        st.markdown(r"Lập bảng biến thiên ta thấy $y'$ đổi dấu từ dương sang âm khi qua $x = 0$. Vậy hàm số đạt cực đại tại $x = 0$, giá trị cực đại là $y_{CĐ} = -2$.")
        
        st.markdown(r"**c) Mệnh đề Đúng:**")
        st.markdown(r"Đường thẳng đi qua 2 điểm cực trị của hàm phân thức $y = \dfrac{u(x)}{v(x)}$ có phương trình là $y = \dfrac{u'(x)}{v'(x)}$.")
        st.markdown(r"Áp dụng vào bài toán: $y = \dfrac{(x^2 - 2x + 2)'}{(x - 1)'} = \dfrac{2x - 2}{1} = 2x - 2$.")
        
        st.markdown(r"**d) Mệnh đề Sai:**")
        st.markdown(r"Hàm số đạt cực tiểu tại điểm $x = 2$, và giá trị cực tiểu (thường gọi tắt là điểm cực tiểu của hàm số) là $y_{CT} = 2$. Mệnh đề cho bằng $0$ là sai.")
    else:
        st.warning("🔒 Vui lòng Đăng nhập ở thanh menu bên trái để xem lời giải chi tiết.")
...
# Lệnh chèn hình ảnh sau đề bài
st.image("images/image_52a53e.png", caption="Đồ thị hàm số Câu 58", use_container_width=True)

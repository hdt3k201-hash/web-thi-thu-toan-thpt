...
# --- Ô NHẬP ĐÁP ÁN VÀ KIỂM TRA ---
user_answer = st.text_input("Nhập độ cao lớn nhất (mét) (ví dụ: 10.6):", key="q76_ans")
...
# --- Ô NHẬP ĐÁP ÁN VÀ KIỂM TRA ---
user_answer = st.text_input("Nhập tổng chi phí thấp nhất (nghìn đồng) :", key="q77_ans")
...
# Tiêu đề câu hỏi
st.markdown(
    '<b style="color: blue;">Câu 78 (THPT Nguyễn Trãi - Hà Nội 2026)</b>',
    unsafe_allow_html=True
)
...
# --- Ô NHẬP ĐÁP ÁN VÀ KIỂM TRA ---
user_answer = st.text_input("Nhập số lượng đường tiệm cận (ví dụ: 1):", key="q78_ans")
...
try:
    ...
    with col2:
        # Đường dẫn ảnh đã được đồng bộ
        st.image("images/image_303d3a.PNG", width=400)
except FileNotFoundError:
    # Thông báo lỗi cập nhật đúng tên file
    st.warning("⚠️ Lỗi: Không tìm thấy file ảnh 'images/image_303d3a.PNG'. Vui lòng kiểm tra lại đường dẫn.")
...
if st.button("Kiểm tra đáp án", key="q78_check"):
    ...
    if normalized_user_answer == "3":
        ...
        st.error("Sai rồi. Hãy tìm tập xác định và xét giới hạn tại các đầu mút của tập xác định để tìm tiệm cận đứng nhé!")
...
if st.session_state.get('q78_solution_shown') and st.session_state.get('logged_in'):
    ...
    **Bước 1: Tìm tập xác định của hàm số**
    ...
    Điều kiện xác định: 
    $$\dfrac{(x-2)^2(x+5)}{(x-4)(x+1)} > 0$$
    
    Vì $(x-2)^2 \ge 0$ với mọi $x$, biểu thức dương khi và chỉ khi:
    $$\begin{cases} x+5 > 0 \\ (x-4)(x+1) > 0 \\ x \neq 2 \end{cases} \iff \begin{cases} x > -5 \\ \left[\begin{array}{l} x < -1 \\ x > 4 \end{array}\right. \\ x \neq 2 \end{cases}$$
    
    Kết hợp các điều kiện trên, ta được tập xác định của hàm số là:
    $$D = (-5; -1) \cup (4; +\infty)$$
    
    **Bước 2: Tìm các đường tiệm cận đứng**
    
    Các đường tiệm cận đứng của đồ thị hàm số xuất hiện tại các biên của tập xác định mà tại đó giới hạn của hàm số là vô cực:
    
    *   Khi $x \to -5^+$, ta có $\dfrac{(x-2)^2(x+5)}{(x-4)(x+1)} \to 0^+$, dẫn đến $\ln \left(\dfrac{(x-2)^2(x+5)}{(x-4)(x+1)}\right) \to -\infty$. 
        Vậy đường thẳng **$x = -5$** là một tiệm cận đứng.
        
    *   Khi $x \to -1^-$, ta có $\dfrac{(x-2)^2(x+5)}{(x-4)(x+1)} \to +\infty$, dẫn đến $\ln \left(\dfrac{(x-2)^2(x+5)}{(x-4)(x+1)}\right) \to +\infty$. 
        Vậy đường thẳng **$x = -1$** là một tiệm cận đứng.
        
    *   Khi $x \to 4^+$, ta có $\dfrac{(x-2)^2(x+5)}{(x-4)(x+1)} \to +\infty$, dẫn đến $\ln \left(\dfrac{(x-2)^2(x+5)}{(x-4)(x+1)}\right) \to +\infty$. 
        Vậy đường thẳng **$x = 4$** là một tiệm cận đứng.
    
    **Bước 3: Tìm các đường tiệm cận ngang**
    
    Xét giới hạn của hàm số khi $x \to +\infty$:
    $$\lim_{x \to +\infty} \ln \left(\dfrac{(x-2)^2(x+5)}{(x-4)(x+1)}\right) = +\infty$$
    Do đó, đồ thị hàm số không có tiệm cận ngang.
    
    **Kết luận:** Đồ thị hàm số có tổng cộng **$3$** đường tiệm cận (đều là tiệm cận đứng).
    """)

...
# Tách chuỗi selected_exam để lấy Tên đề và Chuyên đề
    ...
    with st.form("tsa_exam_form"):
        ...
        if "Đề TSA số 01" in selected_exam:
            ...
            with st.container(border=True):
                ...
                q3_ans = st.text_input(
                    "Nhập kết quả dạng số (Ví dụ: 8):", key="q3"
                )
            ...
            with st.container(border=True):
                ...
                st.markdown(
                    r"""Cho phương trình: $2\cos^2 x - (2m+1)\cos x + m = 0 \quad (1)$."""
                )
                q4_a = st.radio(
        ...
        # =====================================================================
        # NỘI DUNG CÂU HỎI: ĐỀ TSA SỐ 02 (Mẫu cấu trúc để bạn phát triển thêm)
        # =====================================================================
            ...
            with st.container(border=True):
                ...
                st.markdown(
                    r"""Một sự kiện dịch bệnh truyền nhiễm được mô hình hóa bởi hàm số logarit. Đây là câu hỏi ví dụ cho Đề số 2..."""
                )
                st.radio(
                    "Chọn đáp án:",
            ...
            with st.container(border=True):
                ...
                st.markdown(
                    r"""Tính giá trị lớn nhất của hàm số chi phí sản xuất $C(x) = x^3 - 3x^2 + 5$ trên đoạn $[0; 3]$."""
                )
                st.text_input("Nhập kết quả:", key="q_de2_2")

        # =====================================================================
        # NỘI DUNG CÂU HỎI: ĐỀ TSA SỐ 03
        # =====================================================================
            ...
            with st.container(border=True):
                ...
                st.markdown(
                    r"""Cho bảng số liệu về độ tuổi của sinh viên Bách Khoa. Hãy tính số trung vị..."""
                )
                st.radio(
                    "Chọn đáp án:",
                    ["A. 19", "B. 20", "C. 21", "D. 22"],
...
# 7. MÀN HÌNH BÁO KẾT QUẢ VÀ LỜI GIẢI CHI TIẾT
    ...
    if "Đề TSA số 01" in selected_exam:
        if (
            ...
            total_score += 2.5
        if st.session_state.get("q2") and st.session_state.get(
            "q2"
        ).startswith("A."):
            total_score += 2.5
        ...
        c4_score = 0
        if st.session_state.get("q4_1") == "Đúng":
            c4_score += 1
        if st.session_state.get("q4_2") == "Sai":
            c4_score += 1
        if st.session_state.get("q4_3") == "Đúng":
            c4_score += 1
        if st.session_state.get("q4_4") == "Sai":
            c4_score += 1
    ...
    # Hiển thị lời giải theo đề
    if "Đề TSA số 01" in selected_exam:
        with st.expander("🔍 Lời giải Câu 1: Mô hình Vòng quay Mặt Trời"):
            st.markdown(
                r"""- Độ cao tâm vòng quay là vị trí cân bằng $\Rightarrow B = 60$..."""
            )
